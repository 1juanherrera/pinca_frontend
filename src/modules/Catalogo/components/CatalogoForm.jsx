...
const CatalogoForm = ({ item, onSubmit, onClose, isSaving }) => {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      icon={FileText}
      title={isEdit ? 'Editar Ítem' : 'Nuevo Ítem en Catálogo'}
      description="Definición técnica del producto"
      size="2xl"
      footer={
        <>
          <Button variant="secondary" onClick={onClose} disabled={isSaving}>Cancelar</Button>
          <Button type="submit" form="catalogo-form" disabled={isSaving} icon={isSaving ? undefined : FileText} loading={isSaving}>
            {isEdit ? 'Guardar Cambios' : 'Crear en Catálogo'}
          </Button>
        </>
      }
    >
      <form id="catalogo-form" onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
