...
const DataTable = ({ isDrawerOpen, setIsDrawerOpen, items }) => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-zinc-200/80">
              {items.map((item, i) => (
                <tr key={i} className={`border-b border-gray-300 hover:bg-gray-300 
                      relative hover:z-10 hover:ring-2 hover:ring-inset hover:ring-gray-400 transition-colors ${
                      i % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                  }`}>
                  <td className="px-3 py-1.5 text-center text-xs font-medium text-zinc-500">{getId(item)}</td>
                  <td className="px-3 py-1.5"><span className="text-xs font-mono text-zinc-700 font-medium">{getCodigo(item)}</span></td>
                  <td className="px-3 py-1.5 font-semibold text-zinc-900 text-xs">{getNombre(item)}</td>
                  ...
                  <td className="px-3 py-1.5 text-right font-medium text-xs text-emerald-600">{getCostoGalon(item)}</td>
                  <td className="px-3 py-1.5 text-right font-medium text-xs text-blue-600">{getPrecio(item)}</td>
