...
const columns = [
  ...
  },
  {
    ...
    align: 'right',
    render: (v) => (
      <span className="text-xs font-mono tabular-nums font-bold text-amber-700">{fmt(v)}</span>
    ),
  },
];
      ...
      {!isLoading && ordenes.length > 0 && (
        <div className="bg-zinc-900 text-white rounded-xl px-4 py-3 flex items-center justify-between text-xs font-bold">
          ...
          <div className="text-right">
            ...
            <p className="font-mono tabular-nums text-base text-amber-300">{fmt(totalGeneral)}</p>
          </div>
        </div>
      )}
    </div>
